...
class MacDriver extends BaseDriver {
  ...
  mergeCapabilitiesIntoCookies () {
    if (!this.caps.cookies) {
      this.caps.cookies = [];
    }

    // TODO:
    // If 'this.caps.cookies' already has keys/values,
    // this method respect them.

    if (this.caps.implicitTimeout) {
      this.caps.cookies.push({name: 'implicit_timeout', value: this.caps.implicitTimeout});
    }
    if (this.caps.loopDelay) {
      this.caps.cookies.push({name: 'loop_delay', value: this.caps.loopDelay});
    }
    if (this.caps.commandDelay) {
      this.caps.cookies.push({name: 'command_delay', value: this.caps.commandDelay});
    }
    if (this.caps.mouseMoveSpeed) {
      this.caps.cookies.push({name: 'mouse_speed', value: this.caps.mouseMoveSpeed});
    }
    if (this.caps.diagnosticsDirectoryLocation) {
      this.caps.cookies.push({name: 'global_diagnostics_directory', value: this.caps.diagnosticsDirectoryLocation});
    }
    if (this.caps.screenShotOnError) {
      this.caps.cookies.push({name: 'screen_shot_on_error', value: this.caps.screenShotOnError});
    }
  ...
  async startAppiumForMacSession () {
    ...
    this.mergeCapabilitiesIntoCookies();
