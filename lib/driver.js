...
class MacDriver extends BaseDriver {
  ...
  mergeCapabilitiesIntoCookies () {
    ...
    // TODO:
    // If 'this.opts.cookies' already has keys/values,
    // this method respect them.

    if (this.opts.implicitTimeout) {
      this.opts.cookies.push({name: 'implicit_timeout', value: this.opts.implicitTimeout});
    }
    if (this.opts.loopDelay) {
      this.opts.cookies.push({name: 'loop_delay', value: this.opts.loopDelay});
    }
    if (this.opts.commandDelay) {
      this.opts.cookies.push({name: 'command_delay', value: this.opts.commandDelay});
    }
    if (this.opts.mouseMoveSpeed) {
      this.opts.cookies.push({name: 'mouse_speed', value: this.opts.mouseMoveSpeed});
    }
    if (this.opts.diagnosticsDirectoryLocation) {
      this.opts.cookies.push({name: 'global_diagnostics_directory', value: this.opts.diagnosticsDirectoryLocation});
    }
    if (this.opts.screenShotOnError) {
      this.opts.cookies.push({name: 'screen_shot_on_error', value: this.opts.screenShotOnError});
    }
